...
BOOL DataBackupDlg::OnEraseBkgnd(CDC* pDC)
{
	CRect r;
	GetClientRect(&r);
...
HBRUSH DataBackupDlg::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
	...
	if (_tcsicmp(classname, _T("EDIT")) == 0 && pWnd->GetDlgCtrlID() != IDC_EDIT_BK_MSG)
		return hbr;
...
void DataBackupDlg::OnBackup()
{
	...
	CString strMainAppPath = ProductFolderManager::GetProductPath();
