...
void MatLibView::InitMatLibTree()
{
	...
	if (bInternal)
		pMatLibRootNode = MaterialLibLoader::LoadMatLibTree(pMatLibDBCon, FALSE);
	else
		pMatLibRootNode = MaterialLibLoader::LoadMatLibTree(pMatLibDBCon);
...
void MatLibView::OnSelchangedTree(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	else if (m_matLibNodeType == MATLIB_NODE_MATTHICK)
	{
		...
		vector<DataProperty> matSizeProps = MaterialLibLoader::GetMatSizeProp(pMatLibDBCon, pNodeData->GetDataID());
...
void MatLibView::OnRClickTree(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	else if (nodeType == MATLIB_NODE_MATTHICK)
	{
        ...
        // check license, don't remove these code, core dlls also did such check.
