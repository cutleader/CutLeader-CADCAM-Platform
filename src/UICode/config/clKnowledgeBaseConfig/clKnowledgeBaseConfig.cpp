...
BEGIN_MESSAGE_MAP(ClKnowledgeBaseConfigApp, CWinAppEx)
	...
	ON_COMMAND(IDC_HELP_LIC_MGR, &ClKnowledgeBaseConfigApp::OnUpdateDongle)
...
void ClKnowledgeBaseConfigApp::OnUpdateDongle()
{
	CString str, strProductName, strFilter;
	strProductName = MultiLanguageMgr::GetInstance()->TranslateString(IDS_PRODUCT_NAME);
	strFilter = MultiLanguageMgr::GetInstance()->TranslateString(IDS_KEY_FILEFILTER);
	CFileDialog saveDlg(TRUE, NULL, NULL, OFN_HIDEREADONLY|OFN_FILEMUSTEXIST, strFilter, NULL);
	if (saveDlg.DoModal() == IDOK)
	{
		CString strFilePath = saveDlg.GetPathName();
        UpdateDongleResultType iUpdateDongleResult = LicenseManager::UpdateDongle(strFilePath);
        if (iUpdateDongleResult == UpdateDongleResultType_Succeed)
        {
            str = MultiLanguageMgr::GetInstance()->TranslateString(IDS_MSG_WRITE_DONGLE_OK);
        }
        else if (iUpdateDongleResult == UpdateDongleResultType_FailToWrite)
        {
            str = MultiLanguageMgr::GetInstance()->TranslateString(IDS_MSG_WRITE_DONGLE_FAIL);
        }
        else if (iUpdateDongleResult == UpdateDongleResultType_DongleNotAttached)
        {
            str = MultiLanguageMgr::GetInstance()->TranslateString(IDS_MSG_ATTACH_DONGLE);
        }
        else if (iUpdateDongleResult == UpdateDongleResultType_KeyFileError)
        {
            str = MultiLanguageMgr::GetInstance()->TranslateString(IDS_MSG_IMP_KEY_ERROR1);
        }
