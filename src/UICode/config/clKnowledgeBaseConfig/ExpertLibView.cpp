...
void ExpertLibView::InitSubDialog()
{
	...
	m_schemePage.Create(IDD_clExpertLibUI_ParamConfig, this);
...
void ExpertLibView::InitExpLibTree()
{
	...
	m_expLibTree.Init(pExpLibRootNode);
}
	...
	case IDC_NEW_EXPLIBITEM:
		{
			...
			if (dlg.DoModal() == IDOK)
			{
				...
				// create tree node.
				HTREEITEM hExpItemNode = m_expLibTree.AppendExpLibItemNode(pExpLibItemNode);
