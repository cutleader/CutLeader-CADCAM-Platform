...
{
    // 释放grid中资源
    for (unsigned int i = 0; i < m_toolAssignRule_gridList.GetItemCount(); i++)
    {
...
BOOL ToolAssignPage::OnEraseBkgnd(CDC* pDC)
{
	CRect r;
	GetClientRect(&r);
...
void ToolAssignPage::InitDefaultTool(bool bForInnerLoop)
{
    ...
    // 清空combobox中现有数据
    for (unsigned int i = 0; i < m_defaultTool_comboBox.GetCount(); i++)
    {
