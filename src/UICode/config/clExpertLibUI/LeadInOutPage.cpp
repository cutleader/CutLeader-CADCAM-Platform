...
void LeadInOutPage::DoDataExchange(CDataExchange* pDX)
{
    ...
    DDX_Check(pDX, IDC_CHECK_StartFromHoleCenter, m_bStartFromHoleCenter);
    DDX_Control(pDX, IDC_EDIT_MaxHoleSize, m_dMaxHoleSize);
...
BEGIN_MESSAGE_MAP(LeadInOutPage, CResizableDialog)
    ...
    ON_BN_CLICKED(IDC_CHECK_StartFromHoleCenter, OnCheck_StartFromHoleCenter)
    ON_EN_KILLFOCUS(IDC_EDIT_MaxHoleSize, OnKillMaxHoleSize)
...
void LeadInOutPage::OnCheck_StartFromHoleCenter()
{
    if (m_tabCtrl.GetCurSel() == 0)
    {
        UpdateData(TRUE);
        LeadInOutDataPtr pInnerLeadIn = m_pLeadSetting->GetInnerLeadInData();
        pInnerLeadIn->SetIsStartFromHoleCenter(m_bStartFromHoleCenter);
        m_pLeadSetting->SetModified(TRUE);
    }
}

void LeadInOutPage::OnKillMaxHoleSize()
{
    if (m_tabCtrl.GetCurSel() == 0)
    {
        LeadInOutDataPtr pInnerLeadIn = m_pLeadSetting->GetInnerLeadInData();
        if (!MathEx::Equal(pInnerLeadIn->GetMaxHoleSize(), m_dMaxHoleSize.Get_Number()))
        {
            pInnerLeadIn->SetMaxHoleSize(m_dMaxHoleSize.Get_Number());
            m_pLeadSetting->SetModified(TRUE);
        }
    }
}
...
BOOL LeadInOutPage::OnEraseBkgnd(CDC* pDC)
{
	CRect r;
	GetClientRect(&r);
...
void LeadInOutPage::LayoutCtrl()
{
    ...
    AddAnchor(IDC_CHECK_StartFromHoleCenter, TOP_LEFT);
    AddAnchor(IDC_TXT_MaxHoleSize, TOP_LEFT);
    AddAnchor(IDC_EDIT_MaxHoleSize, TOP_LEFT);
	/************************************************************************/
...
void LeadInOutPage::InitLeadInGroup(LeadInOutDataPtr pInnerLeadIn)
{
    ...
    GetDlgItem(IDC_CHECK_StartFromHoleCenter)->EnableWindow(TRUE);
    m_dMaxHoleSize.EnableWindow(TRUE);
    ...
    m_bStartFromHoleCenter = pInnerLeadIn->GetIsStartFromHoleCenter();
    m_dMaxHoleSize.Init(pInnerLeadIn->GetMaxHoleSize(), DIGITAL_NUM, FALSE);
	...
	if (iLeadInType == Lead_Line)
	{
        ...
        {
            GetDlgItem(IDC_CHECK_StartFromHoleCenter)->EnableWindow(FALSE);
            m_dMaxHoleSize.EnableWindow(FALSE);
        }
	}
