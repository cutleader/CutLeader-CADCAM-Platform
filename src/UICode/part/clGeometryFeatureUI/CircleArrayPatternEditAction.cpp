...
CircleArrayPatternEditAction::CircleArrayPatternEditAction(PatternListPtr pPatList, PatternLoopListPtr pPatternLoopList, GlViewPortPtr pViewPort)
{
	m_pPatList = pPatList;
...
BOOL CircleArrayPatternEditAction::LButtonDown(double& dPtX, double& dPtY)
{
	if (m_editState != EDIT_PATTERNCIRCLE_NOTSTART) //EDIT mode
	{
		...
		IPatternPtr pNewPat(new CircleArrayPattern(pPatCirclePat->GetPatternRelationList(),
												basePt, m_data[3], m_data[2], (int)m_data[5]));
	...
	else
	{
		...
		else if(m_drawState == DRAW_ARRAY_SECOND_POINT)
		{
			...
			// enter the selecting pattern action.
			EnterPatSelAction(pPatCirclePat);
		}
	}
...
{
	if (m_editState != EDIT_PATTERNCIRCLE_NOTSTART && !m_pPatList->GetPatternFromID(m_pGripEditPat->GetID()))
	{
