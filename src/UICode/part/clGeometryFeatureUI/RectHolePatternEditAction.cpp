...
RectHolePatternEditAction::RectHolePatternEditAction(PatternListPtr pPatList, PatternLoopListPtr pPatternLoopList, GlViewPortPtr pViewPort)
{
	m_pPatList = pPatList;
...
BOOL RectHolePatternEditAction::LButtonDown(double& dPtX, double& dPtY)
{
	...
	else
	{
		...
		else if(m_drawState == DRAW_RECTHOLE_START_POINT) //second click, we can create a rect hole.
		{
			...
			// create command
			EditorData editorData = GetEditorData();
			ICommandPtr pCommand(new PatternAddCommand(editorData, m_pPatList, m_pPatternLoopList, pRectHolePat));
			pCommand->Do();
			CommandManager::GetInstance()->AddCommand(pCommand);

			// enter the selecting pattern action.
			EnterPatSelAction(pRectHolePat);
		}
	}
...
void RectHolePatternEditAction::UpdateForUndoRedo()
{
	...
	if (m_editState != EDIT_RECTHOLE_NOTSTART && !m_pPatList->GetPatternFromID(m_pGripEditPat->GetID()))
	{
