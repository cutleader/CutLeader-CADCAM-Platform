...
RadSlotPatternEditAction::RadSlotPatternEditAction(PatternListPtr pPatList, PatternLoopListPtr pPatternLoopList, GlViewPortPtr pViewPort)
{
...
BOOL RadSlotPatternEditAction::LButtonDown(double& dPtX, double& dPtY)
{
	if (m_editState != EDIT_RADSLOT_NOTSTART)
	{
		...
		ICommandPtr pCommand(new GripEditCommand(editorData, pRadSlotPat, m_pPatternLoopList, m_pGripEditPat));
		pCommand->Do();
	...
	else
	{
		...
		else if (m_drawStatus == DRAW_RADSLOT_THIRD_POINT)
		{
			...
			// create command
			EditorData editorData = GetEditorData();
			ICommandPtr pCommand(new PatternAddCommand(editorData, m_pPatList, m_pPatternLoopList, pRadSlotPat));
			pCommand->Do();
			CommandManager::GetInstance()->AddCommand(pCommand);

			// enter the selecting pattern action.
			EnterPatSelAction(pRadSlotPat);
		}
	}
...
{
	if (m_editState != EDIT_RADSLOT_NOTSTART && !m_pPatList->GetPatternFromID(m_pGripEditPat->GetID()))
	{
