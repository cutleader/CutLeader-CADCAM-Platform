...
EllipsePatternEditAction::EllipsePatternEditAction(PatternListPtr pPatList, PatternLoopListPtr pPatternLoopList, GlViewPortPtr pViewPort)
{
...
BOOL EllipsePatternEditAction::LButtonDown(double& dPtX, double& dPtY)
{
	if (m_editState != EDIT_ELLIPSE_NOTSTART)
	{
		...
		ICommandPtr pCommand(new GripEditCommand(editorData, pEllipsePat, m_pPatternLoopList, m_pGripEditPat));
		pCommand->Do();
	...
	else
	{
		...
		else if(m_drawStatus == DRAW_ELLIPSE_SECOND_POINT)
		{
			...
			// create command
			EditorData editorData = GetEditorData();
			ICommandPtr pCommand(new PatternAddCommand(editorData, m_pPatList, m_pPatternLoopList, pEllipsePat));
			pCommand->Do();
			CommandManager::GetInstance()->AddCommand(pCommand);

			// enter the selecting pattern action.
			EnterPatSelAction(pEllipsePat);
		}
	}
...
void EllipsePatternEditAction::UpdateForUndoRedo()
{
	...
	if (m_editState != EDIT_ELLIPSE_NOTSTART && !m_pPatList->GetPatternFromID(m_pGripEditPat->GetID()))
	{
