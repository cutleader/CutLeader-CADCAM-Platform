...
ArcArrayPatternEditAction::ArcArrayPatternEditAction(PatternListPtr pPatList, PatternLoopListPtr pPatternLoopList, GlViewPortPtr pViewPort)
{
	m_pPatList = pPatList;
...
BOOL ArcArrayPatternEditAction::LButtonDown(double& dPtX, double& dPtY)
{
	if (m_editState != EDIT_PATTERNARC_NOTSTART) //EDIT mode
	{
		...
		IPatternPtr pNewPat(new ArcArrayPattern(pPatArcPat->GetPatternRelationList(), basePt, m_data[3], m_data[2], 
											 m_data[5], (int)m_data[6]));
...
{
	if (m_editState != EDIT_PATTERNARC_NOTSTART && !m_pPatList->GetPatternFromID(m_pGripEditPat->GetID()))
	{
