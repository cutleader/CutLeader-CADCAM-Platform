...
LineArrayPatternEditAction::LineArrayPatternEditAction(PatternListPtr pPatList, PatternLoopListPtr pPatternLoopList, GlViewPortPtr pViewPort)
{
	m_pPatList = pPatList;
...
BOOL LineArrayPatternEditAction::LButtonDown(double& dPtX, double& dPtY)
{
	...
	if (m_editState != EDIT_PATTERNLINE_NOTSTART)
	{
		...
		LineArrayPattern* pTmpPat = new LineArrayPattern(pPatLinePat->GetPatternRelationList(), 
												     basePt, m_data[2], m_data[3], (int)m_data[4]);
...
{
	if (m_editState != EDIT_PATTERNLINE_NOTSTART && !m_pPatList->GetPatternFromID(m_pGripEditPat->GetID()))
	{
