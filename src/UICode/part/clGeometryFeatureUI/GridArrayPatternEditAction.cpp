...
GridArrayPatternEditAction::GridArrayPatternEditAction(PatternListPtr pPatList, PatternLoopListPtr pPatternLoopList, GlViewPortPtr pViewPort)
{
	m_pPatList = pPatList;
...
BOOL GridArrayPatternEditAction::LButtonDown(double& dPtX, double& dPtY)
{
	if (m_editState != EDIT_PATTERNGRID_NOTSTART) // EDIT mode
	{
		...
		IPatternPtr pNewPat(new GridArrayPattern(pPatGridPat->GetPatternRelationList(), basePt,
											  m_data[3], (int)m_data[5], m_data[2], (int)m_data[4]));
	...
	else
	{
		...
		else if(m_drawState == DRAW_ARRAY_SECOND_POINT)
		{
			...
			// create Pattern-grid pattern.
			Point2D basePt(m_data[0], m_data[1]);
			ArrayPatternBasePtr pPatGridPat(new GridArrayPattern(basePt, m_data[3], (int)m_data[5], 
														   m_data[2], (int)m_data[4]));

			// create command
			EditorData editorData = GetEditorData();
			ICommandPtr pCommand(new ArrayPatternAddCommand(editorData, m_pPatList, m_pPatternLoopList, 
														pPatGridPat, m_pControlPat));
			pCommand->Do();
			CommandManager::GetInstance()->AddCommand(pCommand);
...
{
	if (m_editState != EDIT_PATTERNGRID_NOTSTART && !m_pPatList->GetPatternFromID(m_pGripEditPat->GetID()))
	{
	...
	{
		if (m_pControlPat && !m_pPatList->GetPatternFromID(m_pControlPat->GetID()))
		{
			m_pControlPat.reset();
			m_drawState = DRAW_ARRAY_NOTSTART;
			m_iMsgID = IDS_OUTPUT_PATGRID_PAT;
