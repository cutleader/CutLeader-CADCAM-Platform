...
OBroundPatternEditAction::OBroundPatternEditAction(PatternListPtr pPatList, PatternLoopListPtr pPatternLoopList, GlViewPortPtr pViewPort)
{
...
BOOL OBroundPatternEditAction::LButtonDown(double& dPtX, double& dPtY)
{
	if (m_editState != EDIT_OBROUND_NOTSTART)
	{
		...
		ICommandPtr pCommand(new GripEditCommand(editorData, pOBroundPat, m_pPatternLoopList, m_pGripEditPat));
		pCommand->Do();
	...
	else
	{
		...
		else if(m_drawStatus == DRAW_OBROUND_SECOND_POINT)
		{
			...
			// create command
			EditorData editorData = GetEditorData();
			ICommandPtr pCommand(new PatternAddCommand(editorData, m_pPatList, m_pPatternLoopList, pOBroundPat));
			pCommand->Do();
			CommandManager::GetInstance()->AddCommand(pCommand);

			// enter the selecting pattern action.
			EnterPatSelAction(pOBroundPat);
		}
	}
...
void OBroundPatternEditAction::UpdateForUndoRedo()
{
	...
	if (m_editState != EDIT_OBROUND_NOTSTART && !m_pPatList->GetPatternFromID(m_pGripEditPat->GetID()))
	{
