...
TriangleHolePatternEditAction::TriangleHolePatternEditAction(PatternListPtr pPatList, PatternLoopListPtr pPatternLoopList, GlViewPortPtr pViewPort)
{
...
BOOL TriangleHolePatternEditAction::LButtonDown(double& dPtX, double& dPtY)
{
	if (m_editState != EDIT_TRIANGLE_NOTSTART)
	{
		...
		ICommandPtr pCommand(new GripEditCommand(editorData, pTRHolePat, m_pPatternLoopList, m_pGripEditPat));
		pCommand->Do();
	...
	else
	{
		...
		else if(m_drawState == DRAW_TRIANGLE_SECOND_POINT)
		{
			...
			// create command
			EditorData editorData = GetEditorData();
			ICommandPtr pCommand(new PatternAddCommand(editorData, m_pPatList, m_pPatternLoopList, pTRHolePat));
			pCommand->Do();
			CommandManager::GetInstance()->AddCommand(pCommand);

			// enter the selecting pattern action.
			EnterPatSelAction(pTRHolePat);
		}
	}
...
void TriangleHolePatternEditAction::UpdateForUndoRedo()
{
	...
	if (m_editState != EDIT_TRIANGLE_NOTSTART && !m_pPatList->GetPatternFromID(m_pGripEditPat->GetID()))
	{
