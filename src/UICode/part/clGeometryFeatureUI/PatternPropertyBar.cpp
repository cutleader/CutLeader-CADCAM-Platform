...
PatternPropertyBar* PatternPropertyBar::GetInstance(CWnd* pParent)
{
	if(m_pPatPropBar == NULL)
	{
		...
		if (!m_pPatPropBar->Create(str, (CWnd*)pParent, CRect(0, 0, RIGHT_DOCK_PANE_WIDTH, 200),
								   TRUE, IDC_PROPBAR_PATPROP, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | 
...
BEGIN_MESSAGE_MAP(PatternPropertyBar, CDockablePane)
	ON_WM_CREATE()
	ON_WM_SIZE()
	ON_WM_SETFOCUS()
	ON_WM_SETTINGCHANGE()
...
void PatternPropertyBar::DisplayPatProp(PatternLoopListPtr pPatternLoopList, PatternListPtr pPatList)
{
	...
	else
	{
		...
		for (unsigned int i = 0; i < pPatTypeList->size(); i++)
		{
			...
			VARIANT varGeomTypeName = dataPropItem.GetValue();
...
LRESULT PatternPropertyBar::OnPropertyChange(WPARAM wParam, LPARAM lParam)
{
		...
		item.GetItemName() == IDS_PROP_REPEAT_NUM_Y)
	{
		if (var.intVal < 2)
		{
			...
			DisplayPatProp(m_pPatternLoopList, m_pPatList);
	...
	ICommandPtr pCommand(new PatternPropertyEditCommand(GetEditorData(), m_pPatternLoopList,
											   m_pPatList->at(0), pNewPropItem, pOldPropItem));
	pCommand->Do();
