...
RoundCornerHolePatternEditAction::RoundCornerHolePatternEditAction(PatternListPtr pPatList, PatternLoopListPtr pPatternLoopList, GlViewPortPtr pViewPort)
{
	m_pPatList = pPatList;
...
BOOL RoundCornerHolePatternEditAction::LButtonDown(double& dPtX, double& dPtY)
{
	if (m_editState != EDIT_RCRECT_NOTSTART) //EDIT mode
	{
		...
		ICommandPtr pCommand(new GripEditCommand(editorData, pRCRectPat, m_pPatternLoopList, m_pGripEditPat));
		pCommand->Do();
	...
	else
	{
		...
		else if(m_drawState == DRAW_RCRECT_SECOND_POINT)
		{
			...
			// create command
			EditorData editorData = GetEditorData();
			ICommandPtr pCommand(new PatternAddCommand(editorData, m_pPatList, m_pPatternLoopList, pRCRectPat));
			pCommand->Do();
			CommandManager::GetInstance()->AddCommand(pCommand);

			// enter the selecting pattern action.
			EnterPatSelAction(pRCRectPat);
		}
	}
...
void RoundCornerHolePatternEditAction::UpdateForUndoRedo()
{
	...
	if (m_editState != EDIT_RCRECT_NOTSTART && !m_pPatList->GetPatternFromID(m_pGripEditPat->GetID()))
	{
