...
DoubleDPatternEditAction::DoubleDPatternEditAction(PatternListPtr pPatList, PatternLoopListPtr pPatternLoopList, GlViewPortPtr pViewPort)
{
...
BOOL DoubleDPatternEditAction::LButtonDown(double& dPtX, double& dPtY)
{
	...
	else
	{
		...
		else if(m_drawState == DRAW_DOUBLED_SECOND_POINT) //second click, we can create a line
		{
			...
			// create command
			EditorData editorData = GetEditorData();
			ICommandPtr pCommand(new PatternAddCommand(editorData, m_pPatList, m_pPatternLoopList, pDoubleDPat));
			pCommand->Do();
			CommandManager::GetInstance()->AddCommand(pCommand);

			// enter the selecting pattern action.
			EnterPatSelAction(pDoubleDPat);
		}
	}
...
{
	if (m_editState != EDIT_DOUBLED_NOTSTART && !m_pPatList->GetPatternFromID(m_pGripEditPat->GetID()))
	{
