...
CutSequenceOrderChangeActionA::CutSequenceOrderChangeActionA(PartLayoutDataPtr pPartLayoutData, CutSequenceListPtr pCutSequences,
...
BOOL CutSequenceOrderChangeActionA::LButtonDown(double& dPtX, double& dPtY)
{
	...
	if (m_pCurrentCutSequences->size() > 0)
	{
		...
		if (!m_pBaseCutSequence) // 把选择的工序放到最前面。
		{
			...
			ICommandPtr pCommand(new SequenceLocationMoveCommand(GetEditorData(), m_pCutSequences, m_pConfigData, m_pCurrentCutSequences, pBaseCutSeq, m_pPartLayoutData->GetPartInstances()));
			pCommand->Do();
		...
		else
		{
			...
			// 执行命令。
			ICommandPtr pCommand(new SequenceLocationMoveCommand(GetEditorData(), m_pCutSequences, m_pConfigData, m_pCurrentCutSequences, m_pBaseCutSequence, m_pPartLayoutData->GetPartInstances()));
			pCommand->Do();
			CommandManager::GetInstance()->AddCommand(pCommand);
	...
	if (pCurrentLoopInstance)
	{
		...
		if (m_pApplyPartDlg->IsApplyPart())
		{
			if (pCutPartSeq->GetCutSequenceType() == CutSequence_Bridge || pCutPartSeq->GetCutSequenceType() == CutSequence_ChainCut || pCutPartSeq->GetCutSequenceType() == CutSequence_GridCut)
			{
				...
				m_pCurrentCutSequences->AppendCutSequences(m_pCutSequences->GetCutSequencesByPartInstances(pPartInstances)); // 得到加在这些“零件实例”上的工序。
			}
			else
			{
				...
				if (pOutLoopInst)
				{
					...
					if (pCutPartSeq1 && (pCutPartSeq1->GetCutSequenceType() == CutSequence_Bridge || pCutPartSeq1->GetCutSequenceType() == CutSequence_ChainCut || pCutPartSeq->GetCutSequenceType() == CutSequence_GridCut))
					{
						...
						m_pCurrentCutSequences->AppendCutSequences(m_pCutSequences->GetCutSequencesByPartInstances(pPartInstances)); // 加在这些零件上的工序。
					}
					else
