...
void RemnantCutlineAutoAction::Update(int iEventID, VARIANT varEventContent)
{
	...
	if (m_pGenerateRemCutLineDlg->GetCmdType() == GenerateRemCutLineDlg_previewCmd || // 预览
		m_pGenerateRemCutLineDlg->GetCmdType() == GenerateRemCutLineDlg_generateCmd) // 生成余料切割线
	{
		// 用光照的方式得到余料切割线。
	...
	else if (m_pGenerateRemCutLineDlg->GetCmdType() == GenerateRemCutLineDlg_saveMatCmd) // 保存余料
	{
		// 用光照的方式得到有效区域。
	...
	else if (m_pGenerateRemCutLineDlg->GetCmdType() == GenerateRemCutLineDlg_cancelCmd) // 退出命令
	{
