...
GenerateNCTask::GenerateNCTask(SheetPtr pSheet)
{
	m_pSheet = pSheet;
}
...
void GenerateNCTask::Run() 
{
	...
	for (unsigned int i = 0; i < pPartIDList->size(); i++)
	{
		...
		for (unsigned int j = 0; j < pTmpPartPlacementList->size(); j++)
		{
			...
			{
				// part name.
				strPartName = pPartPlacement->GetPartItem()->GetPartName();

				// part size.
				PartCadDataPtr pPartCadData = pPartPlacement->GetPartItem()->GetCadData();
    ...
    SheetSequenceDataPtr pSheetSequenceData = m_pSheet->GetSheetSequenceData();
    m_pCutNodeList = CutNodeManager::CutSequences2CutNodes_4_simulate(startSeqPt, m_pSheet->GetPartPlacements().get(), m_pSheet->GetPartInstances().get(),
        pSheetSequenceData->GetCutSequences().get(), pLeadConfigItem.get(), pSeqConfigItem.get(), m_patternLoops_failedToOffset);
	/************************************************************************/
	...
	// generate NC code.
	NCGenerator ncGenerator(pPartInfoList, pSheetInfo, pParamConfig, pMacItem);
