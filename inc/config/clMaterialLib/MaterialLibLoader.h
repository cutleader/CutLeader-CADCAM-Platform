...
public:
	...
	// 加载所有材料。
	static MaterialListPtr LoadAllMat(DBConnectPtr pDBConnect, BOOL bLoadVisible = TRUE);

	// 加载材料库所有节点。
	// 注：
	//  1) 只加载可见的材料。
	static IMaterialLibNodePtr LoadMatLibTree(DBConnectPtr pDBConnect, BOOL bLoadVisible = TRUE);

	// 加载指定的材料节点。
	static IMaterialLibNodePtr LoadMatTreeNode(DBConnectPtr pDBConnect, LONGLONG iMatID);
	...
	//      8) 备注
	//      9) 材料尺寸名称。
	static std::vector<DataProperty> GetMatSizeProp(DBConnectPtr pDBConnect, LONGLONG iMatThickID);
...
private:
	...
	static std::map<LONGLONG, std::vector<MatSizeData>> LoadMatSizeData(DBConnectPtr pDBConnect);
	...
	// 注：
	//  1) 返回map的第一个参数是材料厚度ID。
	static std::map<LONGLONG, MaterialLibNodeListPtr> LoadMatSizeNodes(DBConnectPtr pDBConnect);
	/************************************************************************/
};
