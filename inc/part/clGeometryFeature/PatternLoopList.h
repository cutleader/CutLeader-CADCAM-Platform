...
class ClGeometryFeature_Export PatternLoopList : public std::vector<IPatternLoopPtr>,
									   public ClData,
									   public SubjectBase
{
public:
...
public:
	...
	/************************************************************************/
	// about the boundary loop.

	// get the boundary loop.
	int GetBoundaryLoop() const;
	IPatternLoopPtr GetBoundaryLoopEx() const;

	// go through all loops and figure out the boundary loop.
	// notes:
	//   1) the boundary loop must be closed and cannot be array pattern loop.
	//   2) if no boundary loop, return INVALID_IDX.
	int UpdateBoundaryLoop();
	/************************************************************************/
