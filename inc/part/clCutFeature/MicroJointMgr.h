...
// “微连接特征”管理器。
class ClCutFeature_Export MicroJointMgr
{
public:
	// 创建“微连接特征”。
	// 注: 
	//   1) 调用者只要提供微连接点位置和微连接宽度，其他信息来自参数配置。
	static MicroJointPtr BuildMicroJoint(LoopMicroJointPtr pLoopMicroJoint, int iPatIndex, Point2D jointPos, double dSize, MicroJointConfigItemPtr pMicroJointConfigItem);
	...
	// 给回路分配“微连接特征”。
	static void AssignLoopMicroJoint(LoopMicroJointPtr pLoopMicroJoint, MicroJointConfigItemPtr pMicroJointConfigItem);
...
private:
	...
	*/
	...
	static BOOL AssignMicroJointInfo(LoopMicroJointPtr pLoopMicroJoint, MicroJointConfigItemPtr pMicroJointConfigItem, MicroJointRangePtr rangePtr, BOOL& bSkip);
};
