...
public:
	...
	// 创建回路非角上的引入引出线。
	// 注:
	//   1) this function build the lead based on the expert library.
	//   2) also need to assign condition to lead.
	static ILeadPtr BuildGeomLead(const IPatternLoop* pPatternLoop, LeadInOutType emLeadInOut, const LeadConfigItem* pLeadConfigItem);
