...
public:
	...
	*/
	...
	static PartCamDataPtr LoadPartCamData(DBConnectPtr pDBConnect, PartCadDataPtr pPartCadData, 
										  LONGLONG iCamDataID, BOOL bLoadBasic = FALSE);
	...
	*/
	...
	//   2) we do not care the expert library in this function. when importing the part from .CLPrt file, we should make sure we can load cam data of the part successfully.
	//   3) the cache data of returned part item will be updated.
	static PartItemPtr LoadPartItem(DBConnectPtr pDBConnect, LONGLONG iPartID, LONGLONG iPartCamDataID, bool bNeedToLoadPartCamData);
