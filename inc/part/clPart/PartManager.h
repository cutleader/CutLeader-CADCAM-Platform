...
public:
	...
	*/
	...
	static DataItemPtr SaveNewPart(const Part* pPart, FolderNodePtr pParentFolder, BOOL bCheckCAMData = TRUE);
	...
	*/
	...
	// notes:
	//   1) delete the part db and update the main db.
	static void DeletePart(LONGLONG iPartID, BOOL bTransaction = TRUE);

	// update the cache data in part item.
	// notes:
	//   1) this function will update the cache of cad and cam data.
	static void UpdateCacheData(PartItemPtr pPartItem);

	// 计算出零件的预览数据。
	// 注：
	//   1) 如果零件中没有图形，返回空值。
	static ImgDataPtr CalcPreviewData(PartCadDataPtr pPartCadData);
	...
	// 合并CAD数据相同的零件项，这些零件可以用相同的CAD数据和part id。
	// 注：
	//  1) 输入的这些零件项之间没什么关系，都是独立的零件。
	//  2) CAD数据相同，即一个零件通过平移或旋转，能和另一个零件完全贴合起来。
	//  3) 合并后零件项的数量当然不变。
	//  4) matrixs_ofPartItem<part id, matrix list>：比如零件项2、3和零件项1的CAD数据相同，零件项2、3用了零件项1的CAD数据，但是CAD数据的位置可能不一样，此时添加两个矩阵表示零件项2、3。
	//     这两个矩阵都是从零件项1变过来的矩阵，而返回的PartItemList中只有零件项1。
	static PartItemListPtr MergeCadData_of_sameShapePart(const PartItemList* pPartItemList, std::map<LONGLONG, std::vector<Matrix2D>>& matrixData_of_sameShapePart);
