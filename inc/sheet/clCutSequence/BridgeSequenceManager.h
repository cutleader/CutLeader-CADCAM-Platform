...
public:
	...
	*/
	...
	static BridgeSequencePtr ProcessOffset(BridgeSequencePtr pBridgeSeq, PartPlacementListPtr pPartPlacements, PartInstanceListPtr pPartInstances);
	...
	*/
	...
	//  2) 需要替换掉“桥接点”中的图形ID。
	//  3) 这是个递归函数。
	static void UpdateBdgeSeq(BridgeNodePtr pBridgeNode, PartInstanceListPtr pOldPartInsts, PartInstanceListPtr pNewPartInsts);

	// 判断这个桥接工序是不是有效。
	// 注：
	//  1) 需要检测“桥接工序”涉及的“回路实例”还在不在。
	//  2) 需要检测“桥接工序”涉及的“回路实例”的切割方向是不是一样的。
	static BOOL IsBdgeSeqInvalid(BridgeSequencePtr pBridgeSeq, PartInstanceListPtr pAllPartInst);
...
private:
	...
	*/
	...
	static BOOL GetCornerBdgePt(ILineArc2DPtr pCornerDock1, ILineArc2DPtr pCornerDock2, const Point2D& cornerPt, ILineArc2DPtr pEdgeDock, const Point2D& dockPt, double dBdgeWid, BOOL bCCW,
								Point2D& retPt);
