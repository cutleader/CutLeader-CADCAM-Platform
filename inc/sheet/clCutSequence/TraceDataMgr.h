...
public:
	...
	// 构造避让数据。
	static AvoidDataPtr BuildAvoidData(const IMaterialSize* pMaterialSize, const SequenceConfigItem* pSeqConfigItem);
	...
	static Point2DListPtr CalcAvoidPtList(const Point2D& startPt, const Point2D& endPt, const LoopInstanceList* pLoopInstList, AvoidDataPtr pAvoidData, BOOL& bFailAvoid);
	...
	*/
	...
	//   1) if no extra trace line is needed between "startPt" and "endPt", 0 item will be returned.
	//   2) if fail to avoid from "startPt" to "endPt", 0 item will be returned.
	static Point2DListPtr CalcAvoidPtList(const Point2D& startPt, const Point2D& endPt, LineArc2DLoopListPtr pGeomLoopList, const Rect2D& rangeRect, double dAvoidDis, BOOL& bFailAvoid);
	/************************************************************************/
