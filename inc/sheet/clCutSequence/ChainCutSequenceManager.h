...
public:
	...
	static ChainCutSequencePtr ProcessOffset(ChainCutSequencePtr pChainSeq, PartPlacementListPtr pPartPlacements, PartInstanceListPtr pPartInstances);
	...
	static void UpdateChainSeq(ChainCutSequencePtr pChainSeq, PartInstanceListPtr pOldPartInsts, PartInstanceListPtr pNewPartInsts);

	// 判断这个连割工序是不是有效。
	// 注：
	//  1) 需要检测“连割工序”涉及的“回路实例”还在不在。
	//  2) 需要检测“连割工序”涉及的“回路实例”的切割方向是不是一样的。
	static BOOL IsChainSeqInvalid(ChainCutSequencePtr pChainSeq, PartInstanceListPtr pAllPartInst);
