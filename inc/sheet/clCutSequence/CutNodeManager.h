...
public:
	...
	static CutNodeListPtr CutSequences2CutNodes(const Point2D& startCutPt, const CutSequenceList* pCutSequences);
    // 这个函数生成的切割路径和模拟时是一样的，就是最终送往后置的坐标
    static CutNodeListPtr CutSequences2CutNodes_4_simulate(const Point2D& startCutPt, const PartPlacementList* pPartPlacements, const PartInstanceList* pPartInstances,
        const CutSequenceList* pCutSequences, const LeadConfigItem* pLeadConfigItem, const SequenceConfigItem* pSeqConfigItem,
...
private:
	...
	*/
	static CutNodeListPtr Seq2CutNodes(Point2D& lastCutPt, ICutSequencePtr pCutSequence, BOOL bAvoidToNext, LoopInstanceListPtr pCutLoopInsts,
