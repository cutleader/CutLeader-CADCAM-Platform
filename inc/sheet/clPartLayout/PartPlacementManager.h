...
public:
	...
	// 克隆“零件放置”集合。
	// 注：
	//  1) 包括CAD数据在内的其他数据都要克隆。
	//  2) 这个函数不更新缓存。
	static PartPlacementListPtr ClonePartPmtList(const PartPlacementList* pOldPartPmts);

	// 克隆“零件放置”对象。
	// 注：
	//   1) 新的“零件项”中的CAD数据用“pPartCadData”，其中的CAM数据需要克隆，且与“pPartCadData”要对应。
	//   2) 这个函数更新新的“零件项”中的缓存。
	static PartPlacementPtr ClonePartPmt(const PartPlacement* pPartPlacement, PartCadDataPtr pPartCadData);
	...
	*/
	static PartPlacementPtr BuildGridPartPmt(PartPlacementPtr pBasePartPmt, double dPartDis, int iRowNum, int iColNum, PART_GRID_DIR iGridDir);
	...
	*/
	...
	static void ProcessLoopIntf(const PartInstanceTopologyItemList* pPartTopItems, const PartInstanceTopologyItem* pPartTopItem, 
								...
								const LineArc2DList* pAvoidGeoms, const LineArc2DList* pHostGeoms, const LeadConfigItem* pLeadConfigItem,
								BOOL bOnlyScale = FALSE);
	...
	*  polysRect: 零件实例的外接矩形的中心点，用来作为计算临界多边形的参考点。
	*/
	// 注：
	//  1) 这些零件实例作为一个整体移动。
	//  2) 如果“区域边框”无法放下这些零件实例，返回NULL。
	static Polygon2DListPtr CalcInnerStickPolys(RemnantRegionPtr pRemnantRegion, Polygon2DListPtr pPartPolys, const Rect2D& polysRect, double dCompressVal_of_regionBoundary);
	...
	*  polysRect: 零件实例的外接矩形的中心点，用来作为计算临界多边形的参考点。
	*/
	static Polygon2DPtr CalcOuterStickPolys(Polygon2DPtr pRegionHolePoly, Polygon2DListPtr pPartPolys, const Rect2D& polysRect, double dExpandVal_of_regionHole);
	...
	*/
	...
	// replace PartItem involved in "pPartPlacement" with its "brother" PartItem.
	// notes:
	//   1) this function also update the database.
	//   2) this is a dangerous action, after changing the cam data, the layout of the PartPmt may be invalid.
	static void ChangePartCAMData(LONGLONG iSheetID, PartPlacementPtr pPartPlacement, LONGLONG iNewCamDataID, BOOL bTransaction = TRUE);
...
private:
	...
	*/
	...
	static BOOL IsLoopInstIntf(const PartInstanceTopologyItemList* pPartInstTopItems, const PartInstanceTopologyItem* pPartInstTopItem, 
							   LoopInstancePtr pLoopInstance, const std::map<LONGLONG, LineArc2DListPtr>& outerLoopGeom, const LineArc2DList* pAvoidGeoms, const LineArc2DList* pHostGeoms);
	/************************************************************************/
	...
	*/
	// 注：
	//  1) “pPartItem”和“mat”指定了板材上面零件的位置，把此零件按照指定的间距做阵列。返回XY方向的偏移量绝对值。
	...
	static std::pair<double, double> CalcGridData(PartItemPtr pPartItem, Matrix2D mat, double dPartDis);
