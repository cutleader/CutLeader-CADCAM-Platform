...
private:
	...
	*/
	...
	bool AddLabelRptEnt(ReportPagePtr pReportPage, LabelTplEntityPtr pLabelTplEnt, TplEntityListPtr pInnerTplEnts, const std::vector<std::pair<int,int>>& offsets,
						int& iCurRowIndex, int& iCurColumnIndex, PartItemPtr pPartItem);

	// 得到绑定数据的值。
	CString GetBindDataVal(PartItemPtr pPartItem, IBindableEntityPtr pBindableEnt);

	// 得到条形码图形。
	// 注：
	//  1) 如果条码内容为空，返回NULL。
	ImgDataPtr GetBarCodeImg(PartItemPtr pPartItem, BarCodeTplEntityPtr pBarCodeTplEnt);
