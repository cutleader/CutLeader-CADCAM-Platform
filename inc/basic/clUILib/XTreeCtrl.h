...
public:
	...
	*/
	// notes:
	//   1) reference to the illustration, if we selected "folder2,leaf4,leaf5", we only keep "folder2" tree node because it's the parent node.
	//   2) we should not find parent/child relationship in the returned list.
	std::vector<HTREEITEM> GetSelTreeItem();
