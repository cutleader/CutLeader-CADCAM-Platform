...
public:
	NumberProp(const CString& strName, const _variant_t& varValue, CString strFormat,
		      LPCTSTR lpszDescr = NULL, DWORD dwData = 0, BOOL bHasButton = TRUE, BOOL bCanNegative = TRUE);
	NumberProp(const CString& strName, const _variant_t& varValue, int iPrecision,
			   LPCTSTR lpszDescr = NULL, DWORD dwData = 0, BOOL bHasButton = TRUE, BOOL bCanNegative = TRUE);
...
private:
	// we should check the valid of user input and output the formatted value.
	// e.g., 6-(2 is illegal, and 6-2 is legal
