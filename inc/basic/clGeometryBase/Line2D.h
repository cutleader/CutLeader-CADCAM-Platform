...
public: // other functions.
	...
	BOOL CrossWithLine(const Line2D& line) const
	{
		...
		const Point2D& startPt2 = line.GetBasePoint();
		const Point2D& endPt2 = line.GetEndPoint();

		// Exclude ability estimation
		double dMaxX1 = max(startPt1.X(), endPt1.X());
		double dMinX1 = min(startPt1.X(), endPt1.X());
		double dMaxY1 = max(startPt1.Y(), endPt1.Y());
		double dMinY1 = min(startPt1.Y(), endPt1.Y());

		double dMaxX2 = max(startPt2.X(), endPt2.X());
		double dMinX2 = min(startPt2.X(), endPt2.X());
		double dMaxY2 = max(startPt2.Y(), endPt2.Y());
		double dMinY2 = min(startPt2.Y(), endPt2.Y());

		if (dMinX2 > dMaxX1 + SYSTEM_TOLERANCE || dMaxX2 < dMinX1 - SYSTEM_TOLERANCE || 
			dMinY2 > dMaxY1 + SYSTEM_TOLERANCE || dMaxY2 < dMinY1 - SYSTEM_TOLERANCE)
		{
			return FALSE;
		}

		// Cross estimation
		int iSide1 = CheckSide(startPt2);
		int iSide2 = CheckSide(endPt2);
		if (iSide1 * iSide2 > 0)
			return FALSE;

		int iSide3 = line.CheckSide(startPt1);
		int iSide4 = line.CheckSide(endPt1);
		if (iSide3 * iSide4 > 0)
			return FALSE;
