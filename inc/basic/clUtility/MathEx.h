...
public:
	...
	static BOOL GetArcFrom3Point(double dPoint1X, double dPoint1Y, double dPoint2X, double dPoint2Y,
								 double dPoint3X, double dPoint3Y, double& dCenterX, double& dCenterY, 
								 double& dRadius, double& dStartAngle, double& dMoveAngle);

	// get the center point of arc.
	static void GetArcCenter(double dStartPtX, double dStartPtY, double dEndPtX, double dEndPtY,
							 double dSweepAngle, double& dCenterX, double& dCenterY);
	/************************************************************************/
	...
	static void RotatePoint(double dCenterX, double dCenterY, double dAngle, 
							double dOriginX, double dOriginY, double& dPointX, double& dPointY);

	// get the area of the three points.
	/* 参数：
	*  bSigned: 返回值要不要带符号。如果3个点逆时针排布，返回负值。
	*/
	static double GetTriangleArea(double dPtX1, double dPtY1, double dPtX2, double dPtY2, 
								  double dPtX3, double dPtY3, BOOL bSigned);
};
